...
def intra_list_diversity_support(users_partial_lists, items, distance_matrix, k):
    ...
    if k == 1:
        ...
        debug = 2 * distance_matrix[users_partial_lists[:,:k-1,np.newaxis], items].sum(axis=1) / k - old_supp
        return 2 * distance_matrix[users_partial_lists[:,:k-1,np.newaxis], items].sum(axis=1) / k - old_supp
    debug = distance_matrix[users_partial_lists[:,:k-1,np.newaxis], items].sum(axis=1) / (k - 1)
