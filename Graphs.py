...
def print_line_graphs(df, x_axis, x_axis_postfix = False, to_round = 50, folder="OldNormalizationGraphs"):
    ...
    for diversity_type in ["intra_list_diversity","maximal_diversity","binomial_diversity"]:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(folder,f"line_{metrics[1]}_{x_axis}.png"), bbox_inches='tight')
    for novelty_type in ["popularity_complement","maximal_distance_based_novelty","intra_list_distance_based_novelty"]:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(folder,f"line_{metrics[2]}_{x_axis}.png"), bbox_inches='tight')


def correlation(df, folder="OldNormalizationGraphs"):
    for diversity_type in ["intra_list_diversity","maximal_diversity","binomial_diversity"]:
        for novelty_type in ["popularity_complement","maximal_distance_based_novelty","intra_list_distance_based_novelty"]:
