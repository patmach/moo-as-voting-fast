...
class exactly_proportional_fuzzy_dhondt_2:
    ...
    def __call__(self, mask, supports):
        ...
        debug1 = np.take_along_axis(masked_supports, max_gain_items[np.newaxis, :, np.newaxis], axis=2)
        debug = np.squeeze(np.take_along_axis(masked_supports, max_gain_items[np.newaxis, :, np.newaxis], axis=2),axis=2)
